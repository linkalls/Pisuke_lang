...
func TestTypedLetStatement(t *testing.T) {
	...
	if stmt.ValueType == nil {
		t.Fatalf("stmt.ValueType is nil")
	}

	if stmt.ValueType.Value != "int" {
		t.Errorf("stmt.ValueType.Value not 'int'. got=%s", stmt.ValueType.Value)
	}

	val, ok := stmt.Value.(*ast.IntegerLiteral)
	if !ok {
		t.Fatalf("stmt.Value not *ast.IntegerLiteral. got=%T", stmt.Value)
	}

	if val.Value != 10 {
		t.Errorf("val.Value not %d. got=%d", 10, val.Value)
	}
}
...
func TestParsingListLiteral(t *testing.T) {
	input := "[1, 2 * 2, 3 + 3]"
	l := lexer.New(input)
	p := New(l)
	program := p.ParseProgram()
...
func TestParsingMapLiteral(t *testing.T) {
	...
	stmt := program.Statements[0].(*ast.ExpressionStatement)
	mapp, ok := stmt.Expression.(*ast.MapLiteral)
	if !ok {
		t.Fatalf("exp not *ast.MapLiteral. got=%T", stmt.Expression)
	}

	if len(mapp.Pairs) != 3 {
		t.Fatalf("len(mapp.Pairs) not 3. got=%d", len(mapp.Pairs))
	}
	...
	for key, value := range mapp.Pairs {
...
func TestSemicolonError(t *testing.T) {
	input := `let x = 5;`
	l := lexer.New(input)
	p := New(l)
	p.ParseProgram()
	...
	if len(p.Errors) == 0 {
		t.Fatal("parser did not find any errors for semicolon")
	}

	expectedError := `illegal token: ";"`
	if p.Errors[0] != expectedError {
		t.Errorf("wrong error message. expected=%q, got=%q", expectedError, p.Errors[0])
	}
}
...
func TestMemberAccessExpression(t *testing.T) {
	...
	if !ok {
		t.Fatalf("exp is not ast.MemberAccessExpression. got=%T", stmt.Expression)
	}
